	...
	Case!ExpLim[] doIt(DVar v,DExpr e,DExpr x){
		...
		if(auto p=cast(DPow)x){
			DExpr handlePow(DExpr l0,DExpr l1){
				...
				if(l1 == dInf||l1 == -dInf){
					if(l1 == dInf){
						if(dBounded!"()"(l0,mone,one).simplify(facts) == one)
							return zero;
						if(dLt(one,l0).simplify(facts) == one)
							return dInf;
					}
					if(l1 == -dInf){
						if(dBounded!"[]"(l0,mone,one).simplify(facts) == zero)
							return zero;
						if(dBounded!"()"(l0,zero,one).simplify(facts) == one)
							return dInf;
					}
					return null;
				}
